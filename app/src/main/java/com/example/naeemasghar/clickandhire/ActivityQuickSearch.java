...
public class ActivityQuickSearch extends AppCompatActivity {
    private static final String TAG = ActivityQuickSearch.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_activity_quick_search);
        //Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        //setSupportActionBar(toolbar);
        //toolbar.setTitle("Quick Search Professionals");
        ...
        adapterProfessional = new AdapterProfessional(this,professionalList);
        recyclerView.setHasFixedSize(true);
        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(this);
        recyclerView.setLayoutManager(layoutManager);
        recyclerView.setAdapter(adapterProfessional);
        getFilterValues();
        recyclerView.addOnItemTouchListener(new FragmentDashboard.RecyclerTouchListener(this, recyclerView, new FragmentDashboard.ClickListener() {
            @Override
            public void onClick(View view, int position) {
                ModelUser professional = professionalList.get(position);
               // Toast.makeText(getApplicationContext(), professional.getName() + " is selected!", Toast.LENGTH_SHORT).show();
                // Launch Login Activity
                Intent intent = new Intent(ActivityQuickSearch.this, ActivityProfessionalDetail.class);
                String cityName=db.getCityById(professional.getCity());
                intent.putExtra("ratingBool", "0");
    ...
    {
        SharedPreferences prefSearchBy = this.getSharedPreferences(SearchByKey, Context.MODE_PRIVATE);
        SharedPreferences prefCity = this.getSharedPreferences(SearchCity, Context.MODE_PRIVATE);
        prefSearchBy.edit().putInt(SearchByKey, id).apply();
        prefCity.edit().putInt(SearchCity, city).apply();
        if(spf_Id==1)
        {
            txtSearchText.setHint("Write Speciality...");
           // Toast.makeText(getApplicationContext(), "Speciality", Toast.LENGTH_LONG).show();
        }
        else {
            //Toast.makeText(getApplicationContext().getApplicationContext(), "Name", Toast.LENGTH_LONG).show();
            txtSearchText.setHint("Write Name...");
        }
    }
    public void getFilterValues()
    {
        SharedPreferences prefSearchBy = getApplicationContext().getSharedPreferences(SearchByKey, Context.MODE_PRIVATE);
        SharedPreferences prefCity = getApplicationContext().getSharedPreferences(SearchCity, Context.MODE_PRIVATE);
        spf_Id = prefSearchBy.getInt(SearchByKey, 0);
        spf_City = prefCity.getInt(SearchCity, 0);
        if(spf_Id==1)
        {
            txtSearchText.setHint("Write Speciality...");
           // Toast.makeText(getApplicationContext().getApplicationContext(), "Speciality", Toast.LENGTH_LONG).show();
        }
        else {
           // Toast.makeText(getApplicationContext().getApplicationContext(), "Name", Toast.LENGTH_LONG).show();
            txtSearchText.setHint("Write Name...");
        }
       // Toast.makeText(getApplicationContext().getApplicationContext(), "City="+spf_City, Toast.LENGTH_LONG).show();
    ...
    private void getUserByName(final String name, final String cityId){
        class GetUserByName extends AsyncTask<Void,Void,String> {
            ...
            protected void onPreExecute() {
                super.onPreExecute();
                loading = ProgressDialog.show(ActivityQuickSearch.this,"Searching...","Wait...",false,false);
            }
            ...
            protected void onPostExecute(String s) {
                ...
                Toast.makeText(getApplicationContext(), "Search is done Successfully", Toast.LENGTH_LONG).show();
                loading.dismiss();
            }
